...
//
// Resolve symlinks on both sides before comparing: package `bin` shims are
// commonly symlinks (e.g. node_modules/.bin/daax-generate-tests), so the raw
// `process.argv[1]` may point at the shim while `import.meta.url` points at the
// real module file. realpathSync canonicalizes both so the comparison still
// matches; it is wrapped because the path may not exist on disk (e.g. bundled).
...
const invokedPath = process.argv[1] ? canonical(process.argv[1]) : "";
const thisPath = canonical(fileURLToPath(import.meta.url));
